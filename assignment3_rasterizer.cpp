...
void rst::rasterizer::draw(std::vector<Triangle *> &TriangleList) {
    ...
    for (const auto& t:TriangleList)
    {
        ...
        for (auto & vert : v)
        {
            ...
            vert.y() = height - 0.5*height*(vert.y()+1.0);  // Flip Y coordinate
            vert.z() = vert.z() * f1 + f2;
        }
    ...
    for (int x = min_x; x <= max_x; ++x) {
        for (int y = min_y; y <= max_y; ++y) {
            if (insideTriangle(x + 0.5f, y + 0.5f, t.v)){            
                ...
                zp *= Z;
                // Or, in your familar variable names:
                // float w_reciprocal = 1.0/(alpha / v[0].w() + beta / v[1].w() + gamma / v[2].w());
                // float z_interpolated = alpha * v[0].z() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                // z_interpolated *= w_reciprocal;
                // so "z_interpolated" is the zp.


                // TODO: Interpolate the attributes:
                // auto interpolated_color
                
                // float color_interpolated = alpha * v[0].color() / v[0].w() + beta * v[1].z() / v[1].w() + gamma * v[2].z() / v[2].w();
                
                // auto interpolated_normal
                // auto interpolated_texcoords
                // auto interpolated_shadingcoords
                
                // where to get this "v[0].color()"?
                // do the same for "interpolated_normal", "interpolated_texcoords"
